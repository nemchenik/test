...
for old, new in {
    'OUT_DIR = ROOT / "batch36_video_output"': 'OUT_DIR = ROOT / "batch42_video_output"',
    'WORK_DIR = ROOT / "batch36_video_work"': 'WORK_DIR = ROOT / "batch42_video_work"',
    ...
    'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_36_house_plan_facade_video"':
        'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_42_blueprint_grid_video"',
    'CAMPAIGN = "generated_house_plan_facade_video_batch_36"':
        'CAMPAIGN = "generated_blueprint_grid_video_batch_42"',
    'START_PIN = 6512': 'START_PIN = 7712',
    ...
    'f"pinterest/generated_batch_36_house_plan_facade_video/{record.project}.mp4"':
        'f"pinterest/generated_batch_42_blueprint_grid_video/{record.project}.mp4"',
    '"local_validation_batch36.json"': '"local_validation_batch42.json"',
    '"public_validation_batch36.json"': '"public_validation_batch42.json"',
}.items():
...
source = source.replace(
    old_csv_name,
    '"catalog_plans_pinterest_blueprint_grid_videos_batch_42_200.csv"',
)
...
def random_catalog_project_ids() -> list[str]:
    ...
    result.sort(key=lambda value: __import__("hashlib").sha256(("batch42|" + value).encode()).digest())
...
def project_ids() -> list[str]:
    ...
    seed_csv = OUT_DIR / "catalog_plans_pinterest_blueprint_grid_videos_batch_42_200.csv"
    if seed_csv.exists():
        ...
        if len(seeded) != 200 or len(set(seeded)) != 200 or not all(PROJECT_RE.fullmatch(item) for item in seeded):
            raise RuntimeError(f"Invalid batch42 seed CSV: {seed_csv}")
        print("using 200 stable project IDs from the batch42 seed CSV", flush=True)
        return seeded
...
def render_house_slide(record: ProjectMedia, index: int, target: Path) -> None:
    ...
    canvas = Image.new("RGB", (720, 1080), "#102A43")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((0, 0, 18, 1080), fill="#F27A4B")
    draw.text((46, 35), "АРХИТЕКТУРНАЯ КАРТОЧКА", font=font(18, True), fill="#FFFFFF")
    draw.text((42, 94), "01", font=font(66, True), fill="#F27A4B")
    number = f"№ {record.project}"
    number_w = draw.textbbox((0, 0), number, font=font(30, True))[2]
    draw.rectangle((650 - number_w, 28, 680, 82), fill="#F27A4B")
    draw.text((664 - number_w, 40), number, font=font(30, True), fill="#FFFFFF")

    hero = ImageOps.fit(source_image, (602, 502), Image.Resampling.LANCZOS)
    canvas.paste(hero, (76, 176))
    draw.rectangle((68, 168, 686, 686), outline="#FFFFFF", width=8)
    draw.rectangle((76, 626, 296, 678), fill="#F27A4B")
    draw.text((94, 643), "ВИЗУАЛИЗАЦИЯ ДОМА", font=font(14, True), fill="#FFFFFF")
    ...
    draw_footer(draw, record, title_options[index % len(title_options)], "#F27A4B")
...
def draw_header(draw: ImageDraw.ImageDraw, project: str, label: str) -> None:
    draw.rectangle((0, 0, 720, 126), fill="#102A43")
    draw.rectangle((0, 0, 18, 126), fill="#F27A4B")
    draw.rectangle((42, 24, 260, 58), fill="#F27A4B")
    draw.text((54, 32), label, font=font(15, True), fill="#FFFFFF")
    number = f"ПРОЕКТ №{project}"
    width = draw.textbbox((0, 0), number, font=font(27, True))[2]
    draw.text((680 - width, 76), number, font=font(27, True), fill="#FFFFFF")


def draw_footer(draw: ImageDraw.ImageDraw, record: ProjectMedia, title: str, accent: str) -> None:
    draw.rectangle((0, 748, 720, 1080), fill="#102A43")
    draw.rectangle((42, 780, 142, 786), fill=accent)
    draw.text((42, 802), "КРАТКИЙ ОБЗОР", font=font(14, True), fill=accent)
    title_font = font(30, True)
    lines = wrap_text(draw, title, title_font, 630, 2)
    y = 836
    for line in lines:
        draw.text((42, y), line, font=title_font, fill="#FFFFFF")
        y += 38
    chips = []
    if record.area:
    ...
    for chip in chips[:3]:
        ...
        draw.rectangle((x, 954, x + width, 996), outline="#F27A4B", width=2)
        draw.text((x + 14, 965), chip, font=chip_font, fill="#FFFFFF")
        x += width + 8
    draw.line((42, 1020, 680, 1020), fill="#385875", width=1)
    draw.text((42, 1038), "catalog-plans.ru", font=font(15, True), fill="#AFC4D6")
    footer = "визуализация  •  планы  •  фасады"
    width = draw.textbbox((0, 0), footer, font=font(16, True))[2]
    draw.text((680 - width, 1038), footer, font=font(15, True), fill="#FFFFFF")


def render_plan_slide(record: ProjectMedia, plan_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#173B5E")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        draw.rectangle((box[0], box[1], box[0] + box[2], box[1] + box[3]), outline="#FFFFFF", width=4)
        draw.rectangle((box[0], box[1], box[0] + 58, box[1] + 44), fill="#F27A4B")
        draw.text((box[0] + 23, box[1] + 12), str(index), font=font(14, True), fill="#FFFFFF")
    draw.text((42, 718), "02  /  ПЛАНИРОВОЧНЫЕ РЕШЕНИЯ", font=font(16, True), fill="#F27A4B")
    draw_footer(draw, record, "Планы этажей в строгой сетке", "#F27A4B")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
def render_facade_slide(record: ProjectMedia, facade_paths: list[Path], target: Path) -> None:
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        canvas.paste(fitted, (box[0], box[1]))
        draw.rectangle((box[0], box[1], box[0] + box[2], box[1] + box[3]), outline="#FFFFFF", width=4)
        draw.rectangle((box[0], box[1], box[0] + 54, box[1] + 38), fill="#F27A4B")
        draw.text((box[0] + 22, box[1] + 9), f"{index}", font=font(13, True), fill="#FFFFFF")
    draw.text((42, 708), "03  /  ФАСАДЫ СО ВСЕХ СТОРОН", font=font(16, True), fill="#F27A4B")
    draw_footer(draw, record, "Фасады в модульной композиции", "#F27A4B")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
MOTION_NAMES = ["blueprint_house", "modular_floor_plans", "architect_grid", "navy_terracotta"]
...
virtual_path = ROOT / "batch42_blueprint_grid_video_runtime.py"
