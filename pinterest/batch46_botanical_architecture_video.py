...
for old, new in {
    'OUT_DIR = ROOT / "batch36_video_output"': 'OUT_DIR = ROOT / "batch46_video_output"',
    'WORK_DIR = ROOT / "batch36_video_work"': 'WORK_DIR = ROOT / "batch46_video_work"',
    ...
    'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_36_house_plan_facade_video"':
        'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_46_botanical_architecture_video"',
    'CAMPAIGN = "generated_house_plan_facade_video_batch_36"':
        'CAMPAIGN = "generated_botanical_architecture_video_batch_46"',
    'START_PIN = 6512': 'START_PIN = 8512',
    ...
    'f"pinterest/generated_batch_36_house_plan_facade_video/{record.project}.mp4"':
        'f"pinterest/generated_batch_46_botanical_architecture_video/{record.project}.mp4"',
    '"local_validation_batch36.json"': '"local_validation_batch46.json"',
    '"public_validation_batch36.json"': '"public_validation_batch46.json"',
}.items():
...
source = source.replace(
    old_csv_name,
    '"catalog_plans_pinterest_botanical_architecture_videos_batch_46_200.csv"',
)
...
def random_catalog_project_ids() -> list[str]:
    ...
    result.sort(key=lambda value: __import__("hashlib").sha256(("batch46|" + value).encode()).digest())
...
def project_ids() -> list[str]:
    ...
    seed_csv = OUT_DIR / "catalog_plans_pinterest_botanical_architecture_videos_batch_46_200.csv"
    if seed_csv.exists():
        ...
        if len(seeded) != 200 or len(set(seeded)) != 200 or not all(PROJECT_RE.fullmatch(item) for item in seeded):
            raise RuntimeError(f"Invalid batch46 seed CSV: {seed_csv}")
        print("using 200 stable project IDs from the batch46 seed CSV", flush=True)
        return seeded
...
def render_house_slide(record: ProjectMedia, index: int, target: Path) -> None:
    ...
    canvas = Image.new("RGB", (720, 1080), "#F5F3ED")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((34, 0, 38, 1080), fill="#4A7C59")
    draw.text((64, 38), "АРХИТЕКТУРА И ПРИРОДА", font=font(15, True), fill="#4A7C59")
    ...
    draw.text((672 - number_w, 42), number, font=font(28, True), fill="#2D4633")
    draw.line((64, 82, 672, 82), fill="#B7472A", width=1)

    hero = ImageOps.fit(source_image, (580, 520), Image.Resampling.LANCZOS)
    rounded_paste(canvas, hero, (92, 166), radius=42, outline="#4A7C59")
    draw.rounded_rectangle((92, 634, 346, 686), radius=26, fill="#F5F3ED", outline="#4A7C59")
    draw.text((112, 651), "ОБРАЗ ЗАГОРОДНОГО ДОМА", font=font(13, True), fill="#4A7C59")

    title_options = [
        f"Дом {record.area} м²: гармония с природой" if record.area else "Дом в гармонии с природой",
        f"Проект №{record.project}: комфорт и свет",
        "Естественная архитектура дома",
        "Тёплый загородный проект",
    ]
    draw_footer(draw, record, title_options[index % len(title_options)], "#4A7C59")
...
def draw_header(draw: ImageDraw.ImageDraw, project: str, label: str) -> None:
    draw.rectangle((0, 0, 720, 126), fill="#F5F3ED")
    draw.rectangle((34, 0, 38, 126), fill="#4A7C59")
    draw.text((64, 30), label, font=font(15, True), fill="#4A7C59")
    number = f"ПРОЕКТ №{project}"
    width = draw.textbbox((0, 0), number, font=font(26, True))[2]
    draw.text((672 - width, 72), number, font=font(26, True), fill="#2D4633")
    draw.line((64, 106, 672, 106), fill="#B7472A", width=1)


def draw_footer(draw: ImageDraw.ImageDraw, record: ProjectMedia, title: str, accent: str) -> None:
    draw.rectangle((0, 748, 720, 1080), fill="#F5F3ED")
    draw.text((64, 786), "ГАРМОНИЯ ПРОЕКТА", font=font(13, True), fill=accent)
    title_font = font(30, False)
    lines = wrap_text(draw, title, title_font, 630, 2)
    y = 830
    for line in lines:
        draw.text((64, y), line, font=title_font, fill="#2D4633")
        y += 38
    chips = []
    if record.area:
    ...
    for chip in chips[:3]:
        ...
        draw.line((x, 951, x + width, 951), fill="#4A7C59", width=1)
        draw.text((x + 4, 964), chip, font=chip_font, fill="#FFFFFF")
        x += width + 8
    draw.line((64, 1020, 672, 1020), fill="#B7472A", width=1)
    draw.text((64, 1038), "catalog-plans.ru", font=font(15, True), fill="#687C6D")
    footer = "визуализация  •  планы  •  фасады"
    width = draw.textbbox((0, 0), footer, font=font(16, True))[2]
    draw.text((672 - width, 1038), footer, font=font(15, True), fill="#4A7C59")


def render_plan_slide(record: ProjectMedia, plan_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#E7E9DF")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=30, outline="#4A7C59")
        draw.text((box[0] + 18, box[1] + 14), f"{index:02d}", font=font(15, True), fill="#F5F3ED")
    draw.text((64, 716), "II   ПЛАНИРОВОЧНЫЕ РЕШЕНИЯ", font=font(15, True), fill="#4A7C59")
    draw_footer(draw, record, "Комфортная логика пространства", "#4A7C59")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
def render_facade_slide(record: ProjectMedia, facade_paths: list[Path], target: Path) -> None:
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=26, outline="#4A7C59")
        draw.text((box[0] + 18, box[1] + 10), f"{index:02d}", font=font(14, True), fill="#F5F3ED")
    draw.text((64, 706), "III   ФАСАДЫ СО ВСЕХ СТОРОН", font=font(15, True), fill="#4A7C59")
    draw_footer(draw, record, "Архитектура в природной палитре", "#4A7C59")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)

TITLE_PATTERNS = [
    "Дом №{p}: архитектура в природной палитре",
    "Проект №{p}: комфортная планировка",
    "Загородный дом №{p}: полный обзор",
    "Дом №{p}: свет, пространство и фасады",
    "Проект №{p}: гармония формы и природы",
    "Тёплый загородный дом №{p}",
]
OPENERS = [
...
MOTION_NAMES = ["botanical_architecture", "organic_floor_plans", "natural_facades", "green_architecture"]
