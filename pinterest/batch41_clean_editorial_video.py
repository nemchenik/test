...
for old, new in {
    'OUT_DIR = ROOT / "batch36_video_output"': 'OUT_DIR = ROOT / "batch41_video_output"',
    'WORK_DIR = ROOT / "batch36_video_work"': 'WORK_DIR = ROOT / "batch41_video_work"',
    ...
    'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_36_house_plan_facade_video"':
        'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_41_clean_editorial_video"',
    'CAMPAIGN = "generated_house_plan_facade_video_batch_36"':
        'CAMPAIGN = "generated_clean_editorial_video_batch_41"',
    'START_PIN = 6512': 'START_PIN = 7512',
    ...
    'f"pinterest/generated_batch_36_house_plan_facade_video/{record.project}.mp4"':
        'f"pinterest/generated_batch_41_clean_editorial_video/{record.project}.mp4"',
    '"local_validation_batch36.json"': '"local_validation_batch41.json"',
    '"public_validation_batch36.json"': '"public_validation_batch41.json"',
}.items():
...
source = source.replace(
    old_csv_name,
    '"catalog_plans_pinterest_clean_editorial_videos_batch_41_200.csv"',
)
...
def random_catalog_project_ids() -> list[str]:
    ...
    result.sort(key=lambda value: __import__("hashlib").sha256(("batch41|" + value).encode()).digest())
...
def project_ids() -> list[str]:
    ...
    seed_csv = OUT_DIR / "catalog_plans_pinterest_clean_editorial_videos_batch_41_200.csv"
    if seed_csv.exists():
        ...
        if len(seeded) != 200 or len(set(seeded)) != 200 or not all(PROJECT_RE.fullmatch(item) for item in seeded):
            raise RuntimeError(f"Invalid batch41 seed CSV: {seed_csv}")
        print("using 200 stable project IDs from the batch41 seed CSV", flush=True)
        return seeded
...
def render_house_slide(record: ProjectMedia, index: int, target: Path) -> None:
    ...
    canvas = Image.new("RGB", (720, 1080), "#F4F2EC")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((0, 0, 12, 1080), fill="#2F6B5F")
    draw.text((42, 36), "ПРОЕКТ ЧАСТНОГО ДОМА", font=font(18, True), fill="#2F6B5F")
    number = f"№ {record.project}"
    number_w = draw.textbbox((0, 0), number, font=font(30, True))[2]
    draw.rounded_rectangle((654 - number_w, 24, 680, 82), radius=29, fill="#E1E9E4")
    draw.text((666 - number_w, 39), number, font=font(30, True), fill="#28322F")
    draw.line((42, 106, 680, 106), fill="#CDD4CF", width=1)

    hero = ImageOps.fit(source_image, (636, 540), Image.Resampling.LANCZOS)
    rounded_paste(canvas, hero, (42, 142), radius=24, outline="#D2D7D2")
    draw.rounded_rectangle((58, 626, 270, 666), radius=20, fill="#F4F2EC")
    draw.text((76, 636), "ВИЗУАЛИЗАЦИЯ ДОМА", font=font(14, True), fill="#2F6B5F")
    ...
    draw_footer(draw, record, title_options[index % len(title_options)], "#2F6B5F")
...
def draw_header(draw: ImageDraw.ImageDraw, project: str, label: str) -> None:
    draw.rectangle((0, 0, 720, 118), fill="#F4F2EC")
    draw.rectangle((0, 0, 12, 118), fill="#2F6B5F")
    draw.text((42, 25), label, font=font(18, True), fill="#2F6B5F")
    number = f"ПРОЕКТ №{project}"
    width = draw.textbbox((0, 0), number, font=font(27, True))[2]
    draw.text((680 - width, 63), number, font=font(27, True), fill="#28322F")
    draw.line((42, 108, 680, 108), fill="#CDD4CF", width=1)


def draw_footer(draw: ImageDraw.ImageDraw, record: ProjectMedia, title: str, accent: str) -> None:
    draw.rectangle((0, 760, 720, 1080), fill="#F4F2EC")
    draw.text((42, 792), "КРАТКИЙ ОБЗОР", font=font(14, True), fill=accent)
    title_font = font(30, True)
    lines = wrap_text(draw, title, title_font, 630, 2)
    y = 826
    for line in lines:
        draw.text((42, y), line, font=title_font, fill="#28322F")
        y += 38
    chips = []
    if record.area:
    ...
    for chip in chips[:3]:
        ...
        draw.rounded_rectangle((x, 947, x + width, 989), radius=21, fill="#E1E9E4")
        draw.text((x + 14, 958), chip, font=chip_font, fill="#2F6B5F")
        x += width + 8
    draw.line((42, 1018, 680, 1018), fill="#CDD4CF", width=1)
    draw.text((42, 1038), "catalog-plans.ru", font=font(15, True), fill="#65706C")
    footer = "визуализация  •  планы  •  фасады"
    width = draw.textbbox((0, 0), footer, font=font(16, True))[2]
    draw.text((680 - width, 1038), footer, font=font(15, True), fill=accent)


def render_plan_slide(record: ProjectMedia, plan_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#E8EEE9")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=22, outline="#C8D1CB")
        draw.ellipse((box[0] + 14, box[1] + 14, box[0] + 52, box[1] + 52), fill="#2F6B5F")
        draw.text((box[0] + 26, box[1] + 23), str(index), font=font(14, True), fill="#FFFFFF")
    draw.text((42, 720), "ПЛАНИРОВОЧНЫЕ РЕШЕНИЯ", font=font(16, True), fill="#2F6B5F")
    draw_footer(draw, record, "Планы этажей без лишних деталей", "#2F6B5F")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)


def render_facade_slide(record: ProjectMedia, facade_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#EEF0EC")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=18, outline="#C8D1CB")
        draw.rounded_rectangle((box[0] + 12, box[1] + 12, box[0] + 52, box[1] + 42), radius=15, fill="#DCE7E1")
        draw.text((box[0] + 22, box[1] + 19), f"{index}", font=font(13, True), fill="#2F6B5F")
    draw.text((42, 710), "ФАСАДЫ СО ВСЕХ СТОРОН", font=font(16, True), fill="#2F6B5F")
    draw_footer(draw, record, "Четыре стороны одного дома", "#2F6B5F")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
MOTION_NAMES = ["clean_house_review", "minimal_floor_plans", "calm_facades", "light_editorial"]
...
virtual_path = ROOT / "batch41_clean_editorial_video_runtime.py"
