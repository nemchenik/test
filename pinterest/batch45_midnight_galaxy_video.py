...
for old, new in {
    'OUT_DIR = ROOT / "batch36_video_output"': 'OUT_DIR = ROOT / "batch45_video_output"',
    'WORK_DIR = ROOT / "batch36_video_work"': 'WORK_DIR = ROOT / "batch45_video_work"',
    ...
    'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_36_house_plan_facade_video"':
        'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_45_midnight_galaxy_video"',
    'CAMPAIGN = "generated_house_plan_facade_video_batch_36"':
        'CAMPAIGN = "generated_midnight_galaxy_video_batch_45"',
    'START_PIN = 6512': 'START_PIN = 8312',
    ...
    'f"pinterest/generated_batch_36_house_plan_facade_video/{record.project}.mp4"':
        'f"pinterest/generated_batch_45_midnight_galaxy_video/{record.project}.mp4"',
    '"local_validation_batch36.json"': '"local_validation_batch45.json"',
    '"public_validation_batch36.json"': '"public_validation_batch45.json"',
}.items():
...
source = source.replace(
    old_csv_name,
    '"catalog_plans_pinterest_midnight_galaxy_videos_batch_45_200.csv"',
)
...
def random_catalog_project_ids() -> list[str]:
    ...
    result.sort(key=lambda value: __import__("hashlib").sha256(("batch45|" + value).encode()).digest())
...
def project_ids() -> list[str]:
    ...
    seed_csv = OUT_DIR / "catalog_plans_pinterest_midnight_galaxy_videos_batch_45_200.csv"
    if seed_csv.exists():
        ...
        if len(seeded) != 200 or len(set(seeded)) != 200 or not all(PROJECT_RE.fullmatch(item) for item in seeded):
            raise RuntimeError(f"Invalid batch45 seed CSV: {seed_csv}")
        print("using 200 stable project IDs from the batch45 seed CSV", flush=True)
        return seeded
...
def render_house_slide(record: ProjectMedia, index: int, target: Path) -> None:
    ...
    canvas = Image.new("RGB", (720, 1080), "#2B1E3E")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((34, 0, 38, 1080), fill="#A490C2")
    draw.text((64, 38), "КОЛЛЕКЦИЯ АРХИТЕКТУРЫ", font=font(15, True), fill="#A490C2")
    draw.text((64, 92), "I", font=font(54, False), fill="#E6E6FA")
    number = f"№ {record.project}"
    number_w = draw.textbbox((0, 0), number, font=font(28, True))[2]
    draw.text((672 - number_w, 42), number, font=font(28, True), fill="#FFFFFF")
    draw.line((64, 82, 672, 82), fill="#4A4E8F", width=1)

    hero = ImageOps.fit(source_image, (580, 520), Image.Resampling.LANCZOS)
    rounded_paste(canvas, hero, (92, 166), radius=34, outline="#A490C2")
    draw.rounded_rectangle((92, 634, 330, 686), radius=26, fill="#2B1E3E")
    draw.text((112, 651), "АРХИТЕКТУРНЫЙ ОБЛИК", font=font(13, True), fill="#A490C2")

    title_options = [
        f"Дом {record.area} м²: выразительный образ" if record.area else "Выразительный образ дома",
        f"Проект №{record.project}: архитектурная история",
        "Пространство, планы и фасады",
        "Дом с характером в деталях",
    ]
    draw_footer(draw, record, title_options[index % len(title_options)], "#A490C2")
...
def draw_header(draw: ImageDraw.ImageDraw, project: str, label: str) -> None:
    draw.rectangle((0, 0, 720, 126), fill="#2B1E3E")
    draw.rectangle((34, 0, 38, 126), fill="#A490C2")
    draw.text((64, 30), label, font=font(15, True), fill="#A490C2")
    number = f"ПРОЕКТ №{project}"
    width = draw.textbbox((0, 0), number, font=font(26, True))[2]
    draw.text((672 - width, 72), number, font=font(26, True), fill="#FFFFFF")
    draw.line((64, 106, 672, 106), fill="#4A4E8F", width=1)


def draw_footer(draw: ImageDraw.ImageDraw, record: ProjectMedia, title: str, accent: str) -> None:
    draw.rectangle((0, 748, 720, 1080), fill="#2B1E3E")
    draw.text((64, 786), "ПОЛНЫЙ ОБЗОР ПРОЕКТА", font=font(13, True), fill=accent)
    title_font = font(30, False)
    lines = wrap_text(draw, title, title_font, 630, 2)
    y = 830
    for line in lines:
        draw.text((64, y), line, font=title_font, fill="#FFFFFF")
        y += 38
    chips = []
    if record.area:
    ...
    for chip in chips[:3]:
        ...
        draw.line((x, 951, x + width, 951), fill="#A490C2", width=1)
        draw.text((x + 4, 964), chip, font=chip_font, fill="#E6E6FA")
        x += width + 8
    draw.line((64, 1020, 672, 1020), fill="#4A4E8F", width=1)
    draw.text((64, 1038), "catalog-plans.ru", font=font(15, True), fill="#C9C3E2")
    footer = "визуализация  •  планы  •  фасады"
    width = draw.textbbox((0, 0), footer, font=font(16, True))[2]
    draw.text((672 - width, 1038), footer, font=font(15, True), fill="#A490C2")


def render_plan_slide(record: ProjectMedia, plan_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#35345F")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        fitted = fit_contain(image, (box[2], box[3]), padding=22, background="#E6E6FA")
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=28, outline="#A490C2")
        draw.text((box[0] + 18, box[1] + 14), f"{index:02d}", font=font(15, True), fill="#2B1E3E")
    draw.text((64, 716), "II   ПЛАНИРОВОЧНЫЕ РЕШЕНИЯ", font=font(15, True), fill="#A490C2")
    draw_footer(draw, record, "Пространство и сценарии жизни", "#A490C2")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
def render_facade_slide(record: ProjectMedia, facade_paths: list[Path], target: Path) -> None:
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        image = Image.open(path)
        fitted = fit_contain(image, (box[2], box[3]), padding=12, background="#E6E6FA")
        rounded_paste(canvas, fitted, (box[0], box[1]), radius=24, outline="#A490C2")
        draw.text((box[0] + 18, box[1] + 10), f"{index:02d}", font=font(14, True), fill="#2B1E3E")
    draw.text((64, 706), "III   ФАСАДЫ СО ВСЕХ СТОРОН", font=font(15, True), fill="#A490C2")
    draw_footer(draw, record, "Дом со всех сторон", "#A490C2")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)

TITLE_PATTERNS = [
    "Дом №{p}: выразительная архитектура",
    "Проект №{p}: пространство и фасады",
    "Архитектурная история дома №{p}",
    "Дом №{p}: полный обзор планировки",
    "Проект №{p}: фасады крупным планом",
    "Дом с характером №{p}",
]
OPENERS = [
...
MOTION_NAMES = ["midnight_galaxy", "cosmic_floor_plans", "lavender_facades", "midnight_architecture"]
...
virtual_path = ROOT / "batch45_midnight_galaxy_video_runtime.py"
