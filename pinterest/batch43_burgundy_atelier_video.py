...
for old, new in {
    'OUT_DIR = ROOT / "batch36_video_output"': 'OUT_DIR = ROOT / "batch43_video_output"',
    'WORK_DIR = ROOT / "batch36_video_work"': 'WORK_DIR = ROOT / "batch43_video_work"',
    ...
    'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_36_house_plan_facade_video"':
        'VIDEO_DIR = ASSET_CHECKOUT / "pinterest" / "generated_batch_43_burgundy_atelier_video"',
    'CAMPAIGN = "generated_house_plan_facade_video_batch_36"':
        'CAMPAIGN = "generated_burgundy_atelier_video_batch_43"',
    'START_PIN = 6512': 'START_PIN = 7912',
    ...
    'f"pinterest/generated_batch_36_house_plan_facade_video/{record.project}.mp4"':
        'f"pinterest/generated_batch_43_burgundy_atelier_video/{record.project}.mp4"',
    '"local_validation_batch36.json"': '"local_validation_batch43.json"',
    '"public_validation_batch36.json"': '"public_validation_batch43.json"',
}.items():
...
source = source.replace(
    old_csv_name,
    '"catalog_plans_pinterest_burgundy_atelier_videos_batch_43_200.csv"',
)
...
def random_catalog_project_ids() -> list[str]:
    ...
    result.sort(key=lambda value: __import__("hashlib").sha256(("batch43|" + value).encode()).digest())
...
def project_ids() -> list[str]:
    ...
    seed_csv = OUT_DIR / "catalog_plans_pinterest_burgundy_atelier_videos_batch_43_200.csv"
    if seed_csv.exists():
        ...
        if len(seeded) != 200 or len(set(seeded)) != 200 or not all(PROJECT_RE.fullmatch(item) for item in seeded):
            raise RuntimeError(f"Invalid batch43 seed CSV: {seed_csv}")
        print("using 200 stable project IDs from the batch43 seed CSV", flush=True)
        return seeded
...
def render_house_slide(record: ProjectMedia, index: int, target: Path) -> None:
    ...
    canvas = Image.new("RGB", (720, 1080), "#35161D")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((34, 0, 38, 1080), fill="#C6A15B")
    draw.text((64, 38), "КОЛЛЕКЦИЯ ЧАСТНЫХ ДОМОВ", font=font(15, True), fill="#C6A15B")
    draw.text((64, 92), "I", font=font(54, False), fill="#F3EBDD")
    number = f"№ {record.project}"
    number_w = draw.textbbox((0, 0), number, font=font(28, True))[2]
    draw.text((672 - number_w, 42), number, font=font(28, True), fill="#F8F1E6")
    draw.line((64, 82, 672, 82), fill="#765440", width=1)

    hero = ImageOps.fit(source_image, (580, 520), Image.Resampling.LANCZOS)
    canvas.paste(hero, (92, 166))
    draw.rectangle((82, 156, 672, 696), outline="#C6A15B", width=2)
    draw.rectangle((92, 634, 330, 686), fill="#35161D")
    draw.text((112, 651), "ГЛАВНАЯ ВИЗУАЛИЗАЦИЯ", font=font(13, True), fill="#C6A15B")
    ...
    draw_footer(draw, record, title_options[index % len(title_options)], "#C6A15B")
...
def draw_header(draw: ImageDraw.ImageDraw, project: str, label: str) -> None:
    draw.rectangle((0, 0, 720, 126), fill="#35161D")
    draw.rectangle((34, 0, 38, 126), fill="#C6A15B")
    draw.text((64, 30), label, font=font(15, True), fill="#C6A15B")
    number = f"ПРОЕКТ №{project}"
    width = draw.textbbox((0, 0), number, font=font(26, True))[2]
    draw.text((672 - width, 72), number, font=font(26, True), fill="#F8F1E6")
    draw.line((64, 106, 672, 106), fill="#765440", width=1)


def draw_footer(draw: ImageDraw.ImageDraw, record: ProjectMedia, title: str, accent: str) -> None:
    draw.rectangle((0, 748, 720, 1080), fill="#35161D")
    draw.text((64, 786), "АРХИТЕКТУРНОЕ ДОСЬЕ", font=font(13, True), fill=accent)
    title_font = font(30, False)
    lines = wrap_text(draw, title, title_font, 630, 2)
    y = 830
    for line in lines:
        draw.text((64, y), line, font=title_font, fill="#F8F1E6")
        y += 38
    chips = []
    if record.area:
    ...
    for chip in chips[:3]:
        ...
        draw.line((x, 951, x + width, 951), fill="#C6A15B", width=1)
        draw.text((x + 4, 964), chip, font=chip_font, fill="#F3EBDD")
        x += width + 8
    draw.line((64, 1020, 672, 1020), fill="#765440", width=1)
    draw.text((64, 1038), "catalog-plans.ru", font=font(15, True), fill="#BFAFA3")
    footer = "визуализация  •  планы  •  фасады"
    width = draw.textbbox((0, 0), footer, font=font(16, True))[2]
    draw.text((672 - width, 1038), footer, font=font(15, True), fill="#C6A15B")


def render_plan_slide(record: ProjectMedia, plan_paths: list[Path], target: Path) -> None:
    canvas = Image.new("RGB", (720, 1080), "#4B2029")
    draw = ImageDraw.Draw(canvas)
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        ...
        fitted = fit_contain(image, (box[2], box[3]), padding=22, background="#F3EBDD")
        canvas.paste(fitted, (box[0], box[1]))
        canvas.paste(fitted, (box[0], box[1]))
        draw.rectangle((box[0], box[1], box[0] + box[2], box[1] + box[3]), outline="#C6A15B", width=2)
        draw.text((box[0] + 18, box[1] + 14), f"{index:02d}", font=font(15, True), fill="#35161D")
    draw.text((64, 716), "II   ПЛАНИРОВОЧНЫЕ РЕШЕНИЯ", font=font(15, True), fill="#C6A15B")
    draw_footer(draw, record, "Элегантная логика пространства", "#C6A15B")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
def render_facade_slide(record: ProjectMedia, facade_paths: list[Path], target: Path) -> None:
    ...
    for index, (path, box) in enumerate(zip(paths, boxes, strict=False), start=1):
        image = Image.open(path)
        fitted = fit_contain(image, (box[2], box[3]), padding=12, background="#F3EBDD")
        canvas.paste(fitted, (box[0], box[1]))
        canvas.paste(fitted, (box[0], box[1]))
        draw.rectangle((box[0], box[1], box[0] + box[2], box[1] + box[3]), outline="#C6A15B", width=2)
        draw.text((box[0] + 18, box[1] + 10), f"{index:02d}", font=font(14, True), fill="#35161D")
    draw.text((64, 706), "III   ФАСАДЫ СО ВСЕХ СТОРОН", font=font(15, True), fill="#C6A15B")
    draw_footer(draw, record, "Архитектура в четырёх ракурсах", "#C6A15B")
    target.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(target, "PNG", optimize=True)
...
MOTION_NAMES = ["burgundy_atelier", "ivory_floor_plans", "brass_facades", "premium_architecture"]
...
virtual_path = ROOT / "batch43_burgundy_atelier_video_runtime.py"
